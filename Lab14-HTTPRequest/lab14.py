# lab14.py

# Starter code for lab 14 in ICS 32
# Programming with Software Libraries in Python

# Replace the following placeholders with your information.

# HENRY HANLIN ZHENG
# HHZHENG1@UCI.EDU
# 19204536


from abc import ABC, abstractmethod
import random

class Appetite:
    LOW = 3
    MEDIUM = 4
    HIGH = 5

class Dog(ABC):
    def __init__(self, name, age, appetite): 
        self._name = name
        self._age = age
        self.hunger_clock = 0
        self.appetite = appetite

    @abstractmethod
    def breed(self):
        pass

    def name(self):
        pass

    def age(self):
        pass

    def hungry(self):
        """
        The hungry method will check the hungry clock to see if some time has
        passed since the last feeding. If clock is greater than breed typical
        appetite, hunger assessment is randomly selected,
        otherwise hunger clock increases
        """
        if self.hunger_clock > self.appetite:
            return bool(random.getrandbits(1))
        else:
            self.hunger_clock += 1
            return False

    def feed(self):
        """
        Feeds the dog. Hunger clock is reset
        """
        self.hunger_clock = 0

class GermanShepherd(Dog):
    def breed(self):
        return "German Shepherd"

class GoldenRetriever(Dog):
    def breed(self):
        return "Golden Retriever"

class Samoyed(Dog):
    def breed(self):
        return "Samoyed"

if __name__ == '__main__':
    print("Welcome to the Dog Simulator!")
    print("1. German Shepherd")
    print("2. Golden Retriever")
    print("3. Samoyed")
    
    breed = input("Select a breed (1-3): ")
    name = input("What is your dog's name? ")
    try:
        age = int(input("What is your dog's age? "))
    except ValueError:
        print("Invalid input for age. Setting age to 0.")
        age = 0


    if breed == "1":
        dog = GermanShepherd(name, age, Appetite.MEDIUM)
    elif breed == "2":
        dog = GoldenRetriever(name, age, Appetite.HIGH)
    elif breed == "3":
        dog = Samoyed(name, age, Appetite.LOW)
        
    q_flag = False
    while q_flag == False:
        h_text = ""
        if dog.hungry() is False:
            h_text = "not "
        print(f"Your {dog.breed()}, {dog.name()} is {h_text}hungry.")
        feed = input(f"Would you like to feed {dog.name()}? (y/n/q): ")

        if feed == "y":
            dog.feed()
        elif feed == "q":
            break
