...
def get_all(_conn: Connection, _data:any=None) -> BookmarkProtocol:
    '''
    get_all will send a command to the server to retrieve all bookmarks and return the response as a BookmarkProtocol object. 

    The data field of the returned BookmarkProtocol object should be a string representation of a list of all bookmarks.
    '''
    bm = BookmarkProtocol(BookmarkProtocol.LIST, "")
    cmd = BookmarkProtocol.format(bm)
    _write_command(_conn, cmd)
    return BookmarkProtocol.open(_read_command(_conn))
