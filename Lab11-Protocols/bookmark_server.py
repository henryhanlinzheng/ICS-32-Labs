...
def send_all_bookmarks(_conn: bmc.Connection, bm: Bookmarker):
    try:
        notes = bm.read_notes()
        data = "\t".join([n.strip() for n in notes if n.strip()])
        status = bmc.BookmarkProtocol(bmc.BookmarkProtocol.OK, data)
        ...
        with connection:
            ...
            while True:
                ...
                try:
                    ...
                    elif bm_msg.protocol == bmc.BookmarkProtocol.LIST:
                        send_all_bookmarks(_bmc_conn, bm)
                except Exception as ex:
                    bmc.error(_bmc_conn)
                    print(ex)
                    break
